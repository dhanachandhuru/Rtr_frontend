import { Box, Button, FormControl, ImageList, ImageListItem, InputLabel, MenuItem, Select, TextField, TextareaAutosize, useMediaQuery } from '@mui/material';
...
import { uploadClubReportImageToS3 } from '../../helpers/imageUpload';
const UploadReportForm = () => {
    ...
    const handleFormSubmit = async (values,{resetForm}) => {
        try {
            values.year = year
            await axios.post(process.env.REACT_APP_BASE_URL + "api/club/add-report", values)
            toast.dismiss();
                            ...
                            >
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        <MenuItem value={4}>participation</MenuItem>
                                    </Select>
                                </FormControl>
                                <FormControl fullWidth variant="filled"
                                    sx={{ gridColumn: "span 2" }}
                                ...
                                <FormControl fullWidth variant="filled"
                                    sx={{ gridColumn: "span 2" }}
                                >
                                    <InputLabel>Avenue *</InputLabel>
                                    <Select
                                        value={values.avenue}
                                        onChange={handleChange}
                                        name="avenue"
                                        error={!!touched.avenue && !!errors.avenue}
                                        helperText={touched.avenue && errors.avenue}
                                    >
                                        <MenuItem value={1}>Club Service</MenuItem>
                                        <MenuItem value={2}>Professional Service</MenuItem>
                                        <MenuItem value={3}>Community Service</MenuItem>
                                        <MenuItem value={4}>International Service</MenuItem>
                                        <MenuItem value={5}>District Event Participation</MenuItem>
                                        <MenuItem value={6}>District Priorities</MenuItem>
                                        <MenuItem value={7}>Rotary Priorities</MenuItem>
...
const checkoutSchema = yup.object().shape({
    ...
    rotractorsAttended: yup.number().required("Rotaractors Attended is required"),
    rotariansAttended: yup.number().required("Rotarians Attended is required"),
    visitingRotractors: yup.number().required("Visiting Rotaractors is required"),
    guests: yup.number().required("Guests is required"),
    beneficiaries: yup.number().required("Beneficiaries is required"),
    reportType: yup.number().required("Report Type is required"),
    month: yup.number().required("Month is required"),
    avenue: yup.number().required("Avenue is required"),
