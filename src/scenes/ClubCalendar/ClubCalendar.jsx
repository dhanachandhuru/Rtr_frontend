...
const handleStatusViewClose = ()=>{
  ...
  const handleSubmit = async (values, { resetForm }) => {
    try {
      ...
      const { data } = await axios.post(process.env.REACT_APP_BASE_URL + "api/club/add-event", values)
  ...
  const getAllEvents = async () => {
    try {
      const res = await axios.get(process.env.REACT_APP_BASE_URL + "api/club/get-all-events");
  ...
  const handleDelete = async()=>{
    try {
      ...
      if(response){
        ...
        else if (currentStatusData.table == "cabinet"){
  ...
  const validationSchema = Yup.object().shape({
    ...
    eventType:Yup.number().required('This field is required')
  });
                ...
                <Form>
                  ...
                  <Box mb={2}>
                    ...
                    >
                      ...
                      >
                        <MenuItem value={1}>Club Event</MenuItem>
                        <MenuItem value={2}>District Event (Approval Needed)</MenuItem>
                      </Select>
                    </FormControl>
                  </Box>
                  <Box mb={2}>
